---
name: skill-propose
description: Analyze the current project to identify missing skills and propose a new skill to the skill library.
---

<constraints>
- identify a concern not covered by any existing skill before proposing
- the proposed skill must define WHAT must be true, never HOW to achieve it
- the proposed skill must contain no technology-specific references
- the proposed skill must have a single responsibility — one concern only
- the proposed skill must not overlap with any existing skill
- submit the proposal to the skill library
</constraints>

<criteria>
coverage gap identified + proposed skill is abstract and technology-agnostic + single responsibility confirmed + no overlap with existing skills + proposal submitted
</criteria>
